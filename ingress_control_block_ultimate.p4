...
// FSM table and action
    ...
    table fsm {
        ...
        size = 1024;
            ...
            // meta.ing_mir_ses = 27; Commenting for now
			// B. Recirculate
            do_recirculate();
            ...
            // hdr.recir.let_it_go_register_value = 7;	no need	
            // let_it_go_register_key = 0; no need
			}
...
*/
        ...
        const entries = {
            (false,	true,	true,	false,	_,	_, _) :
            original_tcp_packet_action();
            (true,	false,	true,	false,	_,	_, _) :
            original_udp_packet_action();
            (false,	true,	true,	true,	10,	RECIRCULATE, 0x0022) :
			recirculated_mirrored_tcp_packet_accept_action();
	    (true,	false,	true,	true,	10,	RECIRCULATE, 0x0022) :
                        recirculated_mirrored_udp_packet_accept_action();
            (false,	true,	true,	true,	1,	RECIRCULATE, _) :
			original_recirculated_tcp_packet_recirculate_action();
            (false,	true,	true,	true,	1,	ACCEPT, _) :
			original_recirculated_tcp_packet_accept_action();
            (false,	true,	true,	true,	1,	REJECT, _) :
			original_recirculated_tcp_packet_reject_action();
            (true,	false,	true,	true,	1,	RECIRCULATE, _) :
			original_recirculated_udp_packet_recirculate_action();
            (true,	false,	true,	true,	1,	ACCEPT, _) :
			original_recirculated_udp_packet_accept_action();
            (true,	false,	true,	true,	1,	REJECT, _) :
			original_recirculated_udp_packet_reject_action();
            (false,	true,	true,	true,	10,	RECIRCULATE, _) :
			recirculated_mirrored_tcp_packet_recirculate_action();
            (false,	true,	true,	true,	10,	REJECT, _) :
			recirculated_mirrored_tcp_packet_reject_action();
            (false,	true,	false,	true,	10,	REJECT, _) :
			recirculated_mirrored_tcp_packet_reject_action();

            (true,	false,	true,	true,	10,	RECIRCULATE, _) :
            recirculated_mirrored_udp_packet_recirculate_action();
            (true,	false,	true,	true,	10,	REJECT, _) :
	        recirculated_mirrored_udp_packet_reject_action();
            (true,	false,	false,	true,	10,	REJECT, _) :
	        recirculated_mirrored_udp_packet_reject_action();
        }
    ...
    apply {
         ...
         if (hdr.ipv4.isValid()) {
			...
			{
				hdr.tcp.checksum, // in mostly working 
				hdr.ipv4.total_len, // in mostly working 
				hdr.ipv4.src_addr, // in mostly working
				hdr.ipv4.dst_addr,
				hdr.tcp.src_port, // in mostly working 
				hdr.tcp.dst_port, // in mostly working
			...
			//hdr.ipv4.src_addr, // in mostly working
            //hdr.ipv4.dst_addr,
            hdr.udp.src_port, // in mostly working
            hdr.udp.dst_port, // in mostly working 
            hdr.ipv4.total_len, // in mostly working 
            //hdr.ipv4.protocol,
            hdr.udp.checksum // in mostly working
            })[15:0];
			}
...
/*
if(ig_prsr_md.parser_err == 0x0022){
			   recirculated_mirrored_tcp_packet_accept_action();
			}*/
